...
plotly_build.plotly_hash <- function(l = last_plot()) {
  ...
  x$data <- unlist(lapply(l$data, function(d) {
    if (should_eval(d)) {
      ...
      trace_group_cols <- setdiff(colnames(points_df), "dat_index")
      points_df <- dplyr::arrange_(points_df, .dots = c(trace_group_cols, "dat_index")) %>%
                   dplyr::group_by_(.dots = trace_group_cols)
