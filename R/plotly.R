...
plotly_build.plotly_hash <- function(l = last_plot()) {
  ...
  x$data <- unlist(lapply(l$data, function(d) {
    if (should_eval(d)) {
      ...
      points_df <- data.frame(dat_index = seq_along(dat[["x"]])) %>% # indices of the original data elements used in the trace FIXME properly define data length
      ...
      subtrace_group_cols <- setdiff(colnames(points_df), "dat_index")
      trace_group_cols <- setdiff(subtrace_group_cols, "group_index")
      points_df <- dplyr::arrange_(points_df, .dots = c(subtrace_group_cols, "dat_index")) %>%
                   dplyr::group_by_(.dots = subtrace_group_cols)
      points_df$subtrace_index <- dplyr::group_indices(points_df)
      points_df <- dplyr::group_by_(points_df, .dots = trace_group_cols)
      ...
      if (grepl("lines", dat[["mode"]] %||% "markers+lines") && "group_index" %in% subtrace_group_cols) {
      ...
      if (!is.null(trace_point_indices)) {
        ...
        trace_dat_indices <- traces_df$dat_index
      }

      # assign trace names
      names(trace_point_indices) <- sapply(trace_point_indices, function(point_indices){
        paste0(points_df[point_indices[[1]], trace_group_cols], collapse="/")
      })
      ...
      if ("legenditem_index" %in% trace_group_cols) {
